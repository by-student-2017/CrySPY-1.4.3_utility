...
## How to use "plot3d.py", "plot3dn.py", "pplot_v1.py", "pplotc.py" or "plot1d.py"
...
python3 pplot_v1.py
```
- A contour plot showing a pseudo-ternary system when the number of atoms is fixed in a multi-component system, with the remaining components consisting of three elements. The data is obtained from "data/cryspy_rslt".
```
python3 pplotc.py
```
- Use "data/cryspy_rslt" obtained in any calculation to plot a diagram of Structure ID vs. Energy (eV/atom).
```
python3 plot1d.py
```
...
- I also created an example of a Slater-Koster file (skf) using "ase_dftb_1opt_skf_auto_EA-vc". The example shown uses PTBP. The skf file corresponding to the region between La and Lu is not included in PTBP (this point needs to be discussed in more detail in the research paper). Similar to "xTB", the structural optimization does not work well after the 3rd cycle. Therefore, 1 or 2 would be candidates, but for safety, I set "MaxSteps = 1".
