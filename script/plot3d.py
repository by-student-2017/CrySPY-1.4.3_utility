...
sc = ax.scatter(other_points[:,0], other_points[:,1], other_points[:,2],
                c=other_distances, cmap='viridis', s=60, depthshade=True, picker=True, label='Off Hull')

# Hull点と線
if hull_points.size > 0:
    hull_sc = ax.scatter(sorted_hull_points[:,0], sorted_hull_points[:,1], sorted_hull_points[:,2],
                          color='red', s=80, marker='^', picker=True, label='On Hull')
...
def on_pick(event):
    ...
    artist = event.artist
    if artist == sc:
        text = (f"Off Hull -> ID: {other_ids[ind]}, "
                f"HullDist: {other_distances[ind]:.3f} eV/atom\n"
                f"Comp: {format_composition(other_comps[ind])}")
    elif artist == hull_sc:
        text = (f"On Hull -> ID: {hull_ids[sorted_indices][ind]}, "
                f"HullDist: 0.000 eV/atom\n"
                f"Comp: {format_composition(sorted_hull_comps[ind])}")
    else:
        text = ""
    annot.set_text(text)
